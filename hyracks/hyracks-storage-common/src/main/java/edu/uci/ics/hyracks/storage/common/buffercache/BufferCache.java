...
public class BufferCache implements IBufferCacheInternal, ILifeCycleComponent {
	...
	public BufferCache(IIOManager ioManager,
			IPageReplacementStrategy pageReplacementStrategy,
			IPageCleanerPolicy pageCleanerPolicy,
			IFileMapManager fileMapManager, int maxOpenFiles,
			ThreadFactory threadFactory) {
		this.ioManager = ioManager;
		this.pageSize = pageReplacementStrategy.getPageSize();
		this.maxOpenFiles = maxOpenFiles;
		pageReplacementStrategy.setBufferCache(this);
		pageMap = new CacheBucket[pageReplacementStrategy
				.getMaxAllowedNumPages() * MAP_FACTOR];
	...
	private void pinSanityCheck(long dpid) throws HyracksDataException {
		...
		if (fInfo == null && !virtualFiles.contains(fileId)) {
			throw new HyracksDataException("pin called on a fileId " + fileId
	...
	public ICachedPage unpinVirtual(long vpid, long dpid)
			throws HyracksDataException {
		...
		if (!virtPage.virtual) {
			...
			throw new HyracksDataException(
					"First argument must be a virtual page");
		}
		if (virtPage.pinCount.get() <= 1) {
			throw new IllegalStateException(
					"Unpin called on a page that isn't pinned");
		}
		if(DEBUG){
		...
		try {
			...
			System.arraycopy(virtPage.buffer.array(), 0, realPage.getBuffer()
					.array(), 0, virtPage.buffer.capacity());
		} finally {
	...
	public ICachedPage unpinVirtual(ICachedPage vp, long dpid)
			throws HyracksDataException {
		long vpid = ((CachedPage) vp).dpid;
		return unpinVirtual(vpid, dpid);
	}

	private CachedPage findPage(long dpid, boolean virtual)
			throws HyracksDataException {
		while (true) {
			 ...
			 */
			CachedPage victim = (CachedPage) pageReplacementStrategy
					.findVictim();
			if (victim != null) {
				/*
				...
				if (victimHash == hash) {
					...
					try {
						...
						if (victimBucket.cachedPage == victim) {
							...
							while (victimPrev != null
									&& victimPrev.next != victim) {
	...
	private String dumpState() {
		...
		buffer.append("Number of physical pages: ")
				.append(pageReplacementStrategy.getMaxAllowedNumPages())
				.append('\n');
				...
				if (cp != null) {
					...
					while (cp != null) {
						buffer.append("      ").append(cp.cpid).append(" -> [")
								.append(BufferedFileHandle.getFileId(cp.dpid))
								.append(':')
								.append(BufferedFileHandle.getPageId(cp.dpid))
								.append(", ").append(cp.pinCount.get())
								.append(", ")
								.append(cp.valid ? "valid" : "invalid")
								.append(", ")
								.append(cp.virtual ? "virtual" : "physical")
								.append(", ")
								.append(cp.dirty.get() ? "dirty" : "clean")
								.append("]\n");
						cp = cp.next;
						++nCachedPages;
					}
		...
		buffer.append("Number of cached pages: ").append(nCachedPages)
				.append('\n');
	...
	private void read(CachedPage cPage) throws HyracksDataException {
		...
		cPage.buffer.clear();
		ioManager.syncRead(fInfo.getFileHandle(),
				(long) BufferedFileHandle.getPageId(cPage.dpid) * pageSize,
				cPage.buffer);
	}

	BufferedFileHandle getFileInfo(CachedPage cPage)
			throws HyracksDataException {
		...
		public synchronized void run() {
			try {
				while (true) {
					...
					while (true) {
						synchronized (cachedPages) {
							if (curPage >= pageReplacementStrategy
									.getNumPages()) {
								break;
							}
							cleanPage((CachedPage) cachedPages.get(curPage),
									false);
						}
						curPage++;
					}
	...
	public void close() {
		...
		synchronized (fileInfoMap) {
			try {
				for (Map.Entry<Integer, BufferedFileHandle> entry : fileInfoMap
						.entrySet()) {
					boolean fileHasBeenDeleted = entry.getValue()
							.fileHasBeenDeleted();
	...
	public void openFile(int fileId) throws HyracksDataException {
		...
		synchronized (fileInfoMap) {
			...
			if (fInfo == null) {
				...
				while (fileInfoMap.size() >= maxOpenFiles
						&& unreferencedFileFound) {
					...
					for (Map.Entry<Integer, BufferedFileHandle> entry : fileInfoMap
						...
						if (entry.getValue().getReferenceCount() <= 0) {
							...
							if (!fileHasBeenDeleted) {
								ioManager.close(entry.getValue()
										.getFileHandle());
							}
				...
				if (fileInfoMap.size() >= maxOpenFiles) {
					throw new HyracksDataException("Could not open fileId "
							+ fileId + ". Max number of files " + maxOpenFiles
							+ " already opened and referenced.");
				}

				// create, open, and map new file reference
				FileReference fileRef = fileMapManager.lookupFileName(fileId);
				IFileHandle fh = ioManager.open(fileRef,
	...
	private void sweepAndFlush(int fileId, boolean flushDirtyPages)
			throws HyracksDataException {
		for (int i = 0; i < pageMap.length; ++i) {
	...
	private boolean invalidateIfFileIdMatch(int fileId, CachedPage cPage,
			boolean flushDirtyPages) throws HyracksDataException {
		if (BufferedFileHandle.getFileId(cPage.dpid) == fileId) {
			...
			if (pinCount > 0) {
				throw new IllegalStateException("Page "
						...
						+ pinCount + " Page is virtual: " + cPage.virtual);
			}
			cPage.invalidate();
			return true;
		}
	...
	public void closeFile(int fileId) throws HyracksDataException {
		...
		synchronized (fileInfoMap) {
			...
			if (fInfo.decReferenceCount() < 0) {
				throw new HyracksDataException("Closed fileId: " + fileId
						+ " more times than it was opened.");
			}
		}
		if (LOGGER.isLoggable(Level.INFO)) {
	...
	@Override
	public synchronized void deleteFile(int fileId, boolean flushDirtyPages)
			throws HyracksDataException {
		if (LOGGER.isLoggable(Level.INFO)) {
	...
	@Override
	public synchronized void deleteMemFile(int fileId)
			throws HyracksDataException {
		// TODO: possible sanity chcecking here like in above?
		if (LOGGER.isLoggable(Level.INFO)) {
			LOGGER.info("Deleting memory file: " + fileId + " in cache: "
					+ this);
		}
		synchronized (virtualFiles) {
	...
	public ICachedPage confiscatePage(long dpid) throws HyracksDataException {
		while (true) {
			...
			if (victim != null) {
				...
				if (((CachedPage) victim).dpid < 0) { // new page
					returnPage = victim;
					((CachedPage) returnPage).dpid = dpid;
				} else {
					// Case 2a/b
